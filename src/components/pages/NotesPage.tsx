  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ width: 260, flexShrink: 0, borderRight: '1px solid var(--border)', background: 'var(--bg2)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '16px 16px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border2)' }}>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 600 }}>📝 Notes</h3>
          <button onClick={newNote} style={{ width: 28, height: 28, borderRadius: 7, background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.2)', color: 'var(--accent2)', fontSize: 18, cursor: 'pointer', display: 'grid', placeItems: 'center', fontFamily: 'var(--font-mono)' }}>＋</button>
        </div>
        <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--border2)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(59,130,246,0.04)', border: '1px solid var(--border2)', borderRadius: 8, padding: '8px 10px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '8px 14px', borderBottom: '1px solid var(--border2)', flexShrink: 0, flexWrap: 'wrap' }}>
          {['B', 'I', 'U'].map(f => (
            <button key={f} style={{ width: 28, height: 26, borderRadius: 5, background: 'rgba(59,130,246,0.05)', border: '1px solid var(--border2)', color: 'var(--text2)', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 700 }}>{f}</button>
          ))}
          <div style={{ width: 1, height: 16, background: 'var(--border)', margin: '0 4px' }} />
          {['H1', 'H2'].map(t => (
            <button key={t} style={{ height: 26, padding: '0 8px', borderRadius: 5, background: 'rgba(59,130,246,0.05)', border: '1px solid var(--border2)', color: 'var(--text2)', cursor: 'pointer', fontSize: '0.72rem' }}>{t}</button>
          ))}
        ...
        <div style={{ padding: '8px 14px 12px', borderTop: '1px solid var(--border2)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
