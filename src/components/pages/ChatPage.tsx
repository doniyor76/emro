...
export default function ChatPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { type: 'ai', text: "Salom! Men Emro AI yordamchisiman. Xotiralaringiz, rejalaringiz yoki istalgan savol bo'yicha yordam bera olaman. 🌟", time: nowTime() }
  ])
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'rgba(5,7,14,0.98)' }}>
      <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg,var(--accent),var(--accent2))', display: 'grid', placeItems: 'center', boxShadow: '0 0 16px rgba(59,130,246,0.35)' }}>
        ...
        <div>
          <div style={{ fontSize: '0.92rem', fontWeight: 600 }}>Emro AI</div>
      ...
      <div ref={chatRef} style={{ flex: 1, overflowY: 'auto', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', justifyContent: m.type === 'user' ? 'flex-end' : 'flex-start' }}>
            {m.type === 'ai' && (
              <div style={{ width: 30, height: 30, borderRadius: 8, background: 'linear-gradient(135deg,var(--accent),var(--accent2))', display: 'grid', placeItems: 'center', flexShrink: 0, marginTop: 2 }}>
            ...
            <div>
              <div style={{ maxWidth: 320, padding: '10px 14px', borderRadius: m.type === 'ai' ? '4px 16px 16px 16px' : '16px 4px 16px 16px', background: m.type === 'ai' ? 'rgba(15,19,35,0.95)' : 'linear-gradient(135deg,var(--accent),var(--accent2))', border: m.type === 'ai' ? '1px solid rgba(59,130,246,0.1)' : 'none', fontSize: '0.88rem', lineHeight: 1.55, color: 'var(--text)' }}>{m.text}</div>
        ...
        {sending && (
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <div style={{ width: 30, height: 30, borderRadius: 8, background: 'linear-gradient(135deg,var(--accent),var(--accent2))', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
            ...
            <div style={{ padding: '10px 14px', borderRadius: '4px 16px 16px 16px', background: 'var(--card)', border: '1px solid rgba(59,130,246,0.1)', fontSize: '0.78rem', color: 'var(--text3)' }}>AI yozmoqda...</div>
          </div>
        )}
      </div>

      <div style={{ padding: '10px 12px 14px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8, alignItems: 'flex-end', flexShrink: 0 }}>
        <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey} placeholder="AI ga savol bering yoki xotira yozing..." rows={1} style={{ flex: 1, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: 20, padding: '10px 16px', color: 'var(--text)', fontSize: '0.88rem', fontFamily: 'var(--font)', outline: 'none', resize: 'none', maxHeight: 100, lineHeight: 1.4 }} />
        <button onClick={sendMsg} disabled={sending || !input.trim()} style={{ width: 40, height: 40, borderRadius: '50%', background: input.trim() ? 'linear-gradient(135deg,var(--accent),var(--accent2))' : 'rgba(59,130,246,0.1)', border: 'none', cursor: input.trim() ? 'pointer' : 'default', display: 'grid', placeItems: 'center', flexShrink: 0, transition: 'all 0.2s' }}>
