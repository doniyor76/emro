...
export default function LibraryPage() {
  ...
  const STAT_CARDS = [
    { label: 'Jami element', value: stats.total,        color: 'var(--blue2)' },
    { label: 'Rasm',         value: stats.images,       color: 'var(--cyan)'  },
    ...
    <div style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ padding: 24 }}>
        <div style={{ marginBottom: 24 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 400, background: 'linear-gradient(135deg,#fff,var(--cyan))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: 4 }}>📚 Kutubxona</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(120px,1fr))', gap: 10, marginBottom: 28 }}>
          {STAT_CARDS.map(s => (
            <div key={s.label} style={{ background: 'var(--card)', border: '1px solid var(--b1)', borderRadius: 16, padding: '16px 14px' }}>
              <div style={{ fontSize: '1.6rem', fontWeight: 700, fontFamily: 'var(--font-display)', color: s.color, marginBottom: 4 }}>{s.value}</div>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: '0.78rem', color: 'var(--text3)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'var(--font-mono)' }}>
            Kitob javoni <div style={{ flex: 1, height: 1, background: 'var(--b2)' }} />
          </div>
          <div style={{ background: 'var(--card)', border: '1px solid var(--b1)', borderRadius: 20, padding: 20, position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg,var(--blue),var(--cyan),var(--blue))', opacity: 0.4 }} />
